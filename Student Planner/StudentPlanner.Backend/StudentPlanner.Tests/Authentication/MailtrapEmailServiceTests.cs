...
public class MailtrapEmailServiceTests
{
    ...
    [Fact]
    public async Task SendPasswordResetEmailAsync_ShouldThrowApplicationException_WhenSmtpFails()
    {
        var email = "recipient@example.com";
        var token = "secret-token";
        ...
        var exception = await Assert.ThrowsAsync<ApplicationException>(() => _emailService.SendPasswordResetEmailAsync(email, token));
